        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-6">
            ...
            <p className="text-gray-400 text-sm leading-relaxed">
              Discover the world's most incredible destinations with curated experiences designed for modern travelers. Unforgettable journeys start here.
            </p>
            <div className="flex items-center gap-4 pt-2">
              {[Globe, MessageCircle, Camera, Video].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-300 hover:bg-[#0EA5E9] hover:text-white transition-all">
                  <Icon size={18} />
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-gray-400">
              {["About Us", "Destinations", "Tours", "Travel Guides", "Contact"].map((link) => (
                <li key={link}>
                  <Link href="#" className="hover:text-[#0EA5E9] transition-colors">{link}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-gray-400">
              {["FAQ", "Booking Terms", "Privacy Policy", "Refund Policy", "Help Center"].map((link) => (
                <li key={link}>
                  <Link href="#" className="hover:text-[#0EA5E9] transition-colors">{link}</Link>
                </li>
              ))}
